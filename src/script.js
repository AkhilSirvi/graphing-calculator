...
(function () {
    ...
    function createCanvas() {
        ...
        if (rect.width < 100 || rect.height < 100) {
            ...
            const availH = Math.max(200, window.innerHeight - headerH - containerPadding);
    ...
    function drawGrid(ctx) {
        ...
        if (axisYVisible) { const { px } = worldToPixel(0, 0); axisXpx = px; ctx.beginPath(); ctx.moveTo(px + 0.5, 0); ctx.lineTo(px + 0.5, h); ctx.stroke(); }
        if (axisXVisible) { const { py } = worldToPixel(0, 0); axisYpy = py; ctx.beginPath(); ctx.moveTo(0, py + 0.5); ctx.lineTo(w, py + 0.5); ctx.stroke(); }
    ...
    function plotRoutine(f1raw, f2raw) {
        ...
        function makeImplicitEvaluator(raw) {
            ...
            let expr = '(' + parts.slice(0, parts.length - 1).join('=') + ')-(' + parts[parts.length - 1] + ')';
            expr = expr.replace(/\^/g, '**');
            expr = expr.replace(/\bln\b/g, 'Math.log');
            expr = expr.replace(/\bpi\b/gi, 'Math.PI');
            expr = expr.replace(/\be\b/g, 'Math.E');
            expr = expr.replace(/\b(sin|cos|tan|asin|acos|atan|sinh|cosh|tanh|abs|floor|ceil|sqrt|log|max|min|pow|exp)\s*\(/g, 'Math.$1(');
            try { return new Function('x', 'y', 'return (' + expr + ');'); } catch (err) { console.warn('Invalid implicit expression:', raw, err); return null; }
        }
        ...
        function plotImplicit(fn, color = '#000') {
            ...
            const xmin = viewport.xmin, xmax = viewport.xmax, ymin = viewport.ymin, ymax = viewport.ymax;
            ...
            function interp(i1, j1, i2, j2) {
                ...
                if (!isFinite(v1) || !isFinite(v2)) return { x: (x1 + x2) / 2, y: (y1 + y2) / 2 };
                const t = v1 / (v1 - v2);
            ...
            for (let j = 0; j < rows; j++) {
                for (let i = 0; i < cols; i++) {
                    ...
                    segs.forEach(pair => {
                        const pA = edges[pair[0]]; const pB = edges[pair[1]];
        ...
        function makeEvaluator(raw) {
            ...
            expr = expr.replace(/\be\b/g, 'Math.E');
            expr = expr.replace(/\b(sin|cos|tan|asin|acos|atan|sinh|cosh|tanh|abs|floor|ceil|sqrt|log|max|min|pow|exp)\s*\(/g, 'Math.$1(');
            try { return new Function('x', 'return (' + expr + ');'); } catch (err) { console.warn('Invalid expression:', raw, err); return null; }
        }
        ...
        [v1, v2].forEach((v) => { if (!v) return; v.forEach(pt => { if (isFinite(pt.y)) { if (pt.y < ymin) ymin = pt.y; if (pt.y > ymax) ymax = pt.y; } }); });
        if (!isFinite(ymin) || !isFinite(ymax)) { ymin = viewport.ymin; ymax = viewport.ymax; }
        const pad = (ymax - ymin) * 0.1 || 1; ymin -= pad; ymax += pad;

        function plotValues(vals, color) {
            ctx.beginPath(); ctx.lineWidth = 2; ctx.strokeStyle = color; let started = false;
            vals.forEach((pt) => { if (!isFinite(pt.y)) { started = false; return; } const { px, py } = worldToPixel(pt.x, pt.y); if (!started) { ctx.moveTo(px + 0.5, py + 0.5); started = true; } else { ctx.lineTo(px + 0.5, py + 0.5); } });
            ctx.stroke();
        }
        ...
        ctx.fillStyle = 'rgba(0,0,0,0.85)'; ctx.font = '14px sans-serif'; let yText = 18; if (f1raw) { ctx.fillText('f1: ' + f1raw, 12, yText); yText += 18; } if (f2raw) { ctx.fillText('f2: ' + f2raw, 12, yText); }
    }

    function ensureCtx() { const canvas = graphPage.querySelector('canvas'); if (!canvas) { ctx = createCanvas(); drawGrid(ctx); } }

    function clearAndRedraw() { ensureCtx(); drawGrid(ctx); if (lastPlot.f1raw || lastPlot.f2raw) plotRoutine(lastPlot.f1raw, lastPlot.f2raw); }
    ...
    document.addEventListener('click', (e) => {
        ...
        if (action === 'clear') { const f1 = document.getElementById('function1'); const f2 = document.getElementById('function2'); if (f1) f1.value = ''; if (f2) f2.value = ''; lastPlot.f1raw = ''; lastPlot.f2raw = ''; drawGrid(ctx); return; }
        if (action === 'plot') { const f1raw = document.getElementById('function1')?.value || ''; const f2raw = document.getElementById('function2')?.value || ''; lastPlot.f1raw = f1raw; lastPlot.f2raw = f2raw; drawGrid(ctx); plotRoutine(f1raw, f2raw); return; }
        if (value) { const active = document.activeElement; if (active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA')) { const start = active.selectionStart || 0; const end = active.selectionEnd || 0; const text = active.value; const insert = value === 'pi' ? 'Math.PI' : value === 'e' ? 'Math.E' : value; active.value = text.slice(0, start) + insert + text.slice(end); const pos = start + insert.length; active.setSelectionRange(pos, pos); active.focus(); } }
    });

    // pan/zoom handlers
    let isPanning = false; let panStart = null;
    graphPage.addEventListener('pointerdown', (ev) => { ev.preventDefault(); const canvas = graphPage.querySelector('canvas'); if (!canvas) return; isPanning = true; panStart = { x: ev.clientX, y: ev.clientY }; canvas.setPointerCapture(ev.pointerId); });
    graphPage.addEventListener('pointermove', (ev) => { if (!isPanning || !panStart) return; const canvas = graphPage.querySelector('canvas'); if (!canvas) return; const dpr = window.devicePixelRatio || 1; const dx = (ev.clientX - panStart.x); const dy = (ev.clientY - panStart.y); panStart = { x: ev.clientX, y: ev.clientY }; const wpx = canvas.width / dpr; const hpx = canvas.height / dpr; const dxWorld = (-dx / wpx) * (viewport.xmax - viewport.xmin); const dyWorld = (dy / hpx) * (viewport.ymax - viewport.ymin); viewport.xmin += dxWorld; viewport.xmax += dxWorld; viewport.ymin += dyWorld; viewport.ymax += dyWorld; clearAndRedraw(); });
    graphPage.addEventListener('pointerup', (ev) => { const canvas = graphPage.querySelector('canvas'); if (canvas) canvas.releasePointerCapture(ev.pointerId); isPanning = false; panStart = null; });

    // wheel to zoom (center at cursor)
    graphPage.addEventListener('wheel', (ev) => {
        const canvas = graphPage.querySelector('canvas'); if (!canvas) return; ev.preventDefault(); const rect = canvas.getBoundingClientRect(); const px = ev.clientX - rect.left; const py = ev.clientY - rect.top; const { x: wx, y: wy } = pixelToWorld(px, py); const delta = -ev.deltaY; const zoomFactor = Math.exp(delta * 0.0015);
        const nxmin = wx + (viewport.xmin - wx) / zoomFactor; const nxmax = wx + (viewport.xmax - wx) / zoomFactor; const nymin = wy + (viewport.ymin - wy) / zoomFactor; const nymax = wy + (viewport.ymax - wy) / zoomFactor; if (Math.abs(nxmax - nxmin) < 1e-9 || Math.abs(nymax - nymin) < 1e-9) return; viewport.xmin = nxmin; viewport.xmax = nxmax; viewport.ymin = nymin; viewport.ymax = nymax; clearAndRedraw(); }, { passive: false });

    // resize handler
    window.addEventListener('resize', () => { ctx = createCanvas(); clearAndRedraw(); });
})();
