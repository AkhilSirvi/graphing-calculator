...
!(function () {
  ...
  function c(e) {
    function n(t) {
      var e, n;
      return t && t.nodeType
        ? ((e = _t(t).children(".mq-root-block").attr(xt)),
          ...
          (t.init = function (t) {
            ...
            (t.__mathquillify = function (t) {
              ...
              n.createTextarea(),
                ...
                (i.jQ = _t('<span class="mq-root-block"/>')
                  .attr(xt, i.id)
                  .appendTo(s)),
                this.latex(e.text()),
  ...
  function p(t, e) {
    (t.jQadd = function () {
      ...
      (t.reflow = function () {
        ...
        K(this.delimjQs, yt(1 + 0.2 * (t - 1), 1.2), 1.2 * t);
      });
  }
  function d(t, e) {
    var e = e || t,
  ...
  (l.prototype = $t.p),
    ...
    (q = (function () {
      ...
      return function (n, i) {
        ...
        var d,
          ...
          w = wt(i.container || v),
          q = t,
          // track IME composition state for mobile/IME keyboards
          composing = !1;

        // handle generic key events via the existing q-mapping
        w.bind("keydown keypress input keyup focusout paste", function (t) {
          q(t);
        });

        // handle composition and direct input for mobile keyboards / IME
        v.bind("compositionstart", function () {
          composing = !0;
        });

        v.bind("compositionend", function () {
          composing = !1;
          try {
            var val = v.val();
            if (val) {
        ...
        // input event: if not composing, forward typed characters to the controller
        v.bind("input", function (ev) {
          if (composing) return;
          try {
            var val = v.val();
            if (val) {
    ...
    zt.open(function (t, e) {
      (t.exportLatex = function () {
        var latex = this.root.latex().replace(/(\\[a-z]+) (?![a-z])/gi, "$1");
        // remove explicit \left / \right and unescape \( \) so parentheses
        // are handled consistently (e.g. "\sin\left(x\right)" -> "\sin(x)")
        latex = latex.replace(/\\left|\\right/g, "");
        latex = latex.replace(/\\\(|\\\)/g, function (m) {
          return "\\(" === m ? "(" : ")";
        });
        // remove backslashes that escape non-letter characters (e.g. \) -> ))
        latex = latex.replace(/\\([^a-zA-Z])/g, '$1');

        function findMatching(str, idx) {
          var open = str[idx];
          var close = null;
          if (open === "{") close = "}";
          else if (open === "(") close = ")";
        ...
        function convert(s) {
          ...
          for (var i = 0; i < s.length; ) {
            if (s[i] === '\\') {
              ...
              // ignore explicit sizing commands \left and \right so the
              // following delimiter (paren/bracket) is handled normally
              ...
              if (cmd === 'frac') {
                if (s[j] === '{') {
                  var aStart = j + 1;
                  var aEnd = findMatching(s, j);
                  var bStart = aEnd + 2; // should be { after }
                  var bEnd = findMatching(s, aEnd + 1);
                  var a = s.slice(aStart, aEnd);
                  var b = s.slice(bStart, bEnd);
                  out += '(' + convert(a) + ')/(' + convert(b) + ')';
                  i = bEnd + 1;
                  continue;
                }
              } else if (cmd === 'sqrt') {
                // \\sqrt[opt]{arg} or \sqrt{arg}
                if (s[j] === '[') {
                  // skip root index for now
                  var idxStart = j + 1;
                  var idxEnd = findMatching(s, j);
                  j = idxEnd + 1;
                }
                if (s[j] === '{') {
                  var aStart2 = j + 1;
                  var aEnd2 = findMatching(s, j);
                  var a2 = s.slice(aStart2, aEnd2);
                  out += 'sqrt(' + convert(a2) + ')';
                  i = aEnd2 + 1;
                  continue;
                }
                ...
                // general command -> treat as function or constant
                var func = cmd;
                // handle case: command^exp followed by an argument, e.g. \sin^3(x)
                if (s[j] === '^') {
                  // parse exponent
                  var expStart = j + 1;
                  var expEnd, expText;
                  if (s[expStart] === '{') {
                    expEnd = findMatching(s, expStart);
                    expText = s.slice(expStart + 1, expEnd);
                    expEnd = expEnd + 1;
                  } else {
                    expText = s[expStart];
                    expEnd = expStart + 1;
                  }
                  // skip spaces after exponent
                  var p = expEnd;
                  while (p < s.length && s[p] === ' ') p++;
                  // parse following argument (brace or paren or single token)
                  var argText, argEnd;
                  if (s[p] === '{' || s[p] === '(') {
                    argText = s.slice(p + 1, findMatching(s, p));
                    argEnd = findMatching(s, p) + 1;
                  } else if (p < s.length) {
                    // single token argument; also consume any exponent on that argument (e.g. x^3)
                    argText = s[p];
                    argEnd = p + 1;
                    if (argEnd < s.length && s[argEnd] === '^') {
                      var ae = argEnd + 1;
                      if (s[ae] === '{') {
                        var aeEnd = findMatching(s, ae);
                        var aeText = s.slice(ae + 1, aeEnd);
                        argText = argText + '^{' + aeText + '}';
                        argEnd = aeEnd + 1;
                      } else if (ae < s.length) {
                        argText = argText + '^' + s[ae];
                        argEnd = ae + 1;
                      }
                    }
                  } else {
                    // nothing after exponent — treat as function name with exponent
                    out += func + '**(' + convert(expText) + ')';
                    i = expEnd;
                    continue;
                  }
                  out += '(' + func + '(' + convert(argText) + '))**(' + convert(expText) + ')';
                  i = argEnd;
                  continue;
                }
                // if next is { } take as argument
                if (s[j] === '{') {
                  var gStart = j + 1;
                  var gEnd = findMatching(s, j);
                  var inner = s.slice(gStart, gEnd);
                  out += func + '(' + convert(inner) + ')';
                  i = gEnd + 1;
                  continue;
                }
                // if next is a single letter/number, parenthesis, or a backslash-command, use it
                if (j < s.length) {
                  // handle backslash-started command as the argument, e.g. \sin\pi
                  if (s[j] === '\\') {
                    var k = j + 1,
                      cmd2 = '';
                    while (k < s.length && /[a-zA-Z]/.test(s[k])) {
                      cmd2 += s[k];
                      k++;
                    }
                    ...
                    if (cmd2.length) {
                      // convert the command token (e.g. \pi -> pi) and use it as the argument
                      out += func + '(' + convert('\\' + cmd2 + ' ') + ')';
                      i = k;
                      continue;
                    }
                    // otherwise fall through and treat the backslash as stray
                  }

                  if (/[a-zA-Z0-9(]/.test(s[j])) {
                    // grab token or parenthesized argument
                    if (s[j] === '(') {
                      var parStart = j + 1;
                      var parEnd = findMatching(s, j);
                      var par = s.slice(parStart, parEnd);
                      out += func + '(' + convert(par) + ')';
                      i = parEnd + 1;
                      continue;
                    }
                    var tok = s[j];
              ...
              var mEnd = findMatching(s, i);
              var inner2 = s.slice(i + 1, mEnd);
              out += '(' + convert(inner2) + ')';
              i = mEnd + 1;
              continue;
            } else if (s[i] === '^') {
              ...
              var k = i + 1;
              if (s[k] === '{') {
                var eEnd = findMatching(s, k);
                var exp = s.slice(k + 1, eEnd);
                out += '**(' + convert(exp) + ')';
                i = eEnd + 1;
                continue;
              } else {
              ...
              // skip stray escape backslashes; if they escape punctuation, append the punctuation
              i++;
        ...
        // initial conversion
        var converted = convert(latex);

        // simple replacements for common LaTeX symbols to JS equivalents
        converted = converted
          .replace(/\\cdot/g, '*')
          .replace(/\\times/g, '*')
          .replace(/\\pi/g, 'pi')
        ...
        (e.delegateMouseEvents = function () {
          ...
          this.container.bind("mousedown.mathquill", function (n) {
            ...
            n.preventDefault(),
              ...
              u.options.ignoreNextMousedown(n) ||
                ((u.options.ignoreNextMousedown = t),
    ...
    zt.open(function (t) {
      (Bt.p.substituteTextarea = function () {
        return _t(
          "<textarea autocapitalize=off id=textarea1234 autocomplete=off autocorrect=off spellcheck=false x-palm-disable-ste-all=true />"
        )[0];
      }),
      ...
      (t.init = function (t) {
        ...
        (t.selectTowards = function (t, e) {
          ...
          return (
            ...
            (s = n(-t, this.text)),
            i[t] === this
              ? ((o = D(s).createDir(t, e)), (i[t] = o), e.insDirOf(t, o))
              : ((a = this[-t]),
                a
    ...
    (Et.isin = Et["belong"] = i(Q, "\\belong ", "&isin;")),
    ...
    (E = Qt(j, function (t, e) {
      t.createLeftOf = function (t) {
        ...
        ((t[Tt] instanceof A && t[Tt].isItalic !== !1) ||
          (t[Tt] instanceof st &&
            t[Tt][Tt] instanceof A &&
            t[Tt][Tt].isItalic !== !1))
          ? (Et._().createLeftOf(t),
            e.createLeftOf.call(this, t),
    ...
    (A = Qt(k, function (t, e) {
      (t.init = function (t, n) {
        ...
        (t.text = function () {
          ...
          return (
            !this[Tt] ||
              this[Tt] instanceof A ||
              this[Tt] instanceof Q ||
              "\\ " === this[Tt].ctrlSeq ||
              (t = "*" + t),
            !this[Ct] ||
              this[Ct] instanceof Q ||
              this[Ct] instanceof st ||
              (t += "*"),
            t
          );
        });
    })),
    (Bt.p.autoCommands = { sqrt: 1, _maxLength: 4 }),
      ...
      (t.init = function (t) {
        ...
        (t.createLeftOf = function (t) {
          ...
          if (
            ...
            for (
              s = "", r = this, o = 0;
              r instanceof R && r.ctrlSeq === r.letter && i > o;
    ...
    (I = {}),
    (z = Bt.p.autoOperatorNames = { _maxLength: 9 }),
  ...
  (Et.operatorname = Qt(O, function (e) {
    ...
    (G = Qt(Q, function (t, e) {
      (t.init = function () {
        ...
        (t.createLeftOf = function (t) {
          return t[Tt] instanceof U && t[Tt].strict
  ...
  J
    ...
    (Et.overline = Et.bar =
      i(et, "\\overline", "span", 'class="mq-non-leaf mq-overline"')),
    (Et.overrightarrow = i(
      et,
      "\\overrightarrow",
      "span",
            ...
            (function (t, e, n, i) {
              t[e].deleteOutOf = function (s, r) {
                ...
                (t.supsub = n),
                  ...
                  "sub" === e &&
                    _t(t.jQ.addClass("mq-sup-only")[0].lastChild).remove(),
                  this.remove();
              };
            })(
              this,
        ...
        (t.createLeftOf = function (t) {
          e.createLeftOf.apply(this, arguments),
            t.options.sumStartsWithNEquals &&
              (R("n").createLeftOf(t), G().createLeftOf(t));
        }),
        (t.latex = function () {
          function t(t) {
        ...
        (e.oppBrack = function (t, e, n) {
          return (
            ...
            (!t.restrictMismatchedBrackets ||
              ft[this.sides[this.side].ch] === e.sides[e.side].ch ||
    ...
    // Build default autoCommands from the Et (commands) table so typing
    // symbol names (e.g. "sqrt", "pi", "alpha") auto-substitutes.
    (function () {
      try {
        var _ac = {}, _max = 0;
        for (var _k in Et)
          if (
            Et.hasOwnProperty(_k) &&
            /^[A-Za-z]+$/.test(_k) &&
            _k.length > 1
          ) {
            _ac[_k] = 1;
            if (_k.length > _max) _max = _k.length;
          }
        _ac._maxLength = _max;
        Bt.p.autoCommands = _ac;
      } catch (e) {}
    })();
